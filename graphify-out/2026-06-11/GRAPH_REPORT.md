...
3. `useData()` - 39 edges
4. `useLogs()` - 35 edges
5. `logValueForHabit()` - 32 edges
6. `useAppDate()` - 27 edges
7. `habitScore()` - 26 edges
8. `useHabits()` - 25 edges
9. `todayKey()` - 24 edges
10. `Goal` - 24 edges
...
- `HabitDayEditorSheetProps` --references--> `Habit`  [EXTRACTED]
  apps/web/src/components/habit/HabitDayEditorSheet.tsx → packages/core/src/types.ts
- `HabitReminderModalProps` --references--> `Habit`  [EXTRACTED]
  apps/web/src/components/habit/HabitReminderModal.tsx → packages/core/src/types.ts
...
Cohesion: 0.07
Nodes (27): dependencies, @dnd-kit/core, @dnd-kit/modifiers, @dnd-kit/sortable, framer-motion, html-to-image, lottie-react, @mottazen/core (+19 more)
...
Cohesion: 0.10
Nodes (20): Activity Model Plan for future implementation, B1. Abstinence ("quit X"), B2. Allowance / tolerance budget, B3. Conditional / scheduled activation, Cross-cutting structural mismatches, Design invariants (hold the line), Gap analysis (paper item → true nature → fit), Naming / vocabulary (decided) (+12 more)
...
Cohesion: 0.12
Nodes (15): 1. Comeback design (smallest, highest emotional ROI), 2. Milestone share cards, 3. Year/Month in Habits (Wrapped), 4. Streak gardens, 5. Identity mode, Architectural context, Design invariants (non-negotiable), Feature map: effort and privacy fit (+7 more)
